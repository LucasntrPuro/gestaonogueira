...
// --- LOGIN (RESTRITO POR STATUS DA LOJA) ---
async function fazerLogin() {
    ...
    const { data: loja } = await _supabase
        .from('lojas')
        .select('ativo')
        .eq('id', data.loja_id)
        .single();
    ...
    if (loja && loja.ativo === false) {
        return alert("⚠️ ESTA EMPRESA ESTÁ BLOQUEADA!");
    }
    ...
    // MONITOR DE EXPULSÃO: Verifica a cada 30 segundos se a loja foi bloqueada
    setInterval(async () => {
        if (usuarioLogado) {
            const { data: check } = await _supabase.from('lojas').select('ativo').eq('id', usuarioLogado.loja_id).single();
            if (check && check.ativo === false) {
                alert("⚠️ A empresa foi bloqueada agora. Saindo...");
                location.reload(); 
            }
        }
    }, 30000);
...
// --- ABAIXO TODAS AS SUAS FUNÇÕES ORIGINAIS SEM ALTERAÇÃO ---
...
function renderCarrinho() {
    ...
    carrinho.forEach((item, i) => {
        ...
        t += sub;
        tbody.innerHTML += `<tr><td>${item.tipo}</td><td>${item.qtd_venda}</td><td>R$ ${item.preco.toFixed(2)}</td><td>R$ ${sub.toFixed(2)}</td><td><button onclick="abrirModalEditarItem(${i})">✏️</button><button onclick="removerItemCarrinho(${i})">❌</button></td></tr>`;
    });
...
async function finalizarVenda() {
    ...
    try {
        ...
        if(!error) {
            ...
            carrinho = []; renderCarrinho(); document.getElementById('venda-cliente').value = "";
        } else { throw error; }
...
async function salvarProduto() {
    ...
    const d = { codigo_barras: document.getElementById('cad-codigo').value, tipo: document.getElementById('cad-tipo').value, preco: parseFloat(document.getElementById('cad-preco').value), quantidade: parseInt(document.getElementById('cad-qtd').value), loja_id: usuarioLogado.loja_id };
    if(id) { await _supabase.from('produtos').update(d).eq('id', id).eq('loja_id', usuarioLogado.loja_id); } 
...
async function carregarHistorico() {
    ...
    let query = _supabase.from('historico_vendas').select('*').eq('loja_id', usuarioLogado.loja_id).order('data_venda', {ascending: false});
...
async function carregarUsuarios() {
    const { data } = await _supabase.from('usuarios').select('*').eq('loja_id', usuarioLogado.loja_id).order('login');
    const tbody = document.getElementById('corpo-usuarios');
    tbody.innerHTML = "";
    data?.forEach(u => {
        const cor = u.ativo ? '#2ecc71' : '#e74c3c';
        tbody.innerHTML += `<tr><td>${u.login}</td><td>${u.nivel}</td><td><span style="color:${cor}">●</span> ${u.ativo?'Ativo':'Inativo'}</td><td><button onclick='prepararEdicaoUsuario(${JSON.stringify(u)})'>✏️</button><button onclick="excluirUsuario(${u.id})">🗑️</button></td></tr>`;
    });
}

async function salvarUsuario() {
    const id = document.getElementById('edit-id-usuario').value;
    const d = { login: document.getElementById('user-login').value, senha: document.getElementById('user-senha').value, nivel: document.getElementById('user-nivel').value, ativo: document.getElementById('user-status').value === "true", loja_id: usuarioLogado.loja_id };
    if(id) { await _supabase.from('usuarios').update(d).eq('id', id).eq('loja_id', usuarioLogado.loja_id); } 
    else { await _supabase.from('usuarios').insert([d]); }
    fecharModalUsuario(); carregarUsuarios();
}

function abrirModalProduto() { document.getElementById('edit-id-produto').value = ""; document.getElementById('cad-codigo').value = ""; document.getElementById('cad-tipo').value = ""; document.getElementById('cad-preco').value = ""; document.getElementById('cad-qtd').value = ""; document.getElementById('modal-produto').style.display='flex'; }
function abrirModalUsuario() { document.getElementById('edit-id-usuario').value = ""; document.getElementById('user-login').value = ""; document.getElementById('user-senha').value = ""; document.getElementById('modal-usuario').style.display='flex'; }
...
function abrirModalEditarItem(index) { const item = carrinho[index]; document.getElementById('edit-carrinho-index').value = index; document.getElementById('edit-carrinho-nome').value = item.tipo; document.getElementById('edit-carrinho-qtd').value = item.qtd_venda; document.getElementById('edit-carrinho-preco').value = item.preco; document.getElementById('modal-editar-item').style.display = 'flex'; }
function salvarEdicaoCarrinho() { const index = document.getElementById('edit-carrinho-index').value; const novaQtd = parseInt(document.getElementById('edit-carrinho-qtd').value); const novoPreco = parseFloat(document.getElementById('edit-carrinho-preco').value); if (novaQtd > 0 && !isNaN(novoPreco)) { carrinho[index].qtd_venda = novaQtd; carrinho[index].preco = novoPreco; fecharModalCarrinho(); renderCarrinho(); } }
...
async function excluirVenda(id) { 
    if(confirm("Excluir esta venda e estornar produtos?")) { 
        ...
        await _supabase.from('historico_vendas').delete().eq('id', id).eq('loja_id', usuarioLogado.loja_id); carregarHistorico(); 
    } 
}
...
async function atualizarPresenca() { if (usuarioLogado?.id) { await _supabase.from('usuarios').update({ ultima_atividade: new Date().toISOString() }).eq('id', usuarioLogado.id); } }
