...
async function fazerLogin() {
    ...
    document.getElementById('user-info').innerHTML = `Usuário: <b>${data.login.toUpperCase()}</b> (${data.nivel})`;
    
    // Aplica restrições de nível imediatamente
...
function configurarPermissoes() {
    ...
    // Esconde abas de gerente na sidebar
...
function renderCarrinho() {
    ...
    carrinho.forEach((item, i) => {
        ...
        total += sub;
        tbody.innerHTML += `<tr><td>${item.tipo}</td><td>${item.qtd_venda}</td><td>R$ ${item.preco.toFixed(2)}</td><td>R$ ${sub.toFixed(2)}</td><td><button onclick="removerItemCarrinho(${i})" style="background:transparent">❌</button></td></tr>`;
    });
...
async function carregarEstoque() {
    ...
    data.forEach(p => {
        tbody.innerHTML += `<tr>
            ...
            <td>${p.quantidade}</td>
            <td>
                ${eGerente ? `<button onclick='editarProduto(${JSON.stringify(p)})'>✏️</button>
...
async function salvarProduto() {
    if(usuarioLogado.nivel !== 'gerente') return alert("Apenas gerentes alteram o estoque!");
...
// --- HISTÓRICO E ESTORNO (SOMENTE GERENTE) ---
async function carregarHistorico() {
    ...
    data.forEach(v => {
        tbody.innerHTML += `<tr>
            ...
            <td>${eGerente ? `<button onclick="excluirVenda(${v.id})" style="background:#ff4d4d">Estornar</button>` : 'Bloqueado'}</td>
        </tr>`;
    });
}

async function excluirVenda(id) {
    if(usuarioLogado.nivel !== 'gerente') return alert("Acesso negado para estorno!");
    if(!confirm("Deseja estornar e devolver itens ao estoque?")) return;
...
// --- USUÁRIOS (CORREÇÃO DO LOJA_ID NULL) ---
