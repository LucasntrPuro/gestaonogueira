...
async function fazerLogin() {
    ...
    document.getElementById('user-info').innerHTML = `<b style="color:#d4af37">${data.lojas?.nome_loja.toUpperCase() || 'LOJA'}</b><br>Operador: ${data.login.toUpperCase()}`;
...
async function carregarHistorico() {
    ...
    tbody.innerHTML = ""; let soma = 0;
    data?.forEach(v => {
        soma += v.total;
        tbody.innerHTML += `<tr><td>${new Date(v.data_venda).toLocaleDateString()}</td><td>${v.cliente}</td><td>${v.produtos}</td><td>R$ ${v.total.toFixed(2)}</td><td><button class="somente-gerente" onclick="excluirVenda(${v.id})">🗑️</button></td></tr>`;
    });
